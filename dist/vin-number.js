"use strict";
(self["webpackChunk_adobe_helix_project_boilerplate"] = self["webpackChunk_adobe_helix_project_boilerplate"] || []).push([["vin-number"],{

/***/ "./blocks/vin-number/vin-number.js":
/*!*****************************************!*\
  !*** ./blocks/vin-number/vin-number.js ***!
  \*****************************************/
...
function renderRecalls(recallsData) {
  ...
  let resultContent = (0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('result text').replace(/\${count}/, recallsData.number_of_recalls).replace(/\${vin}/, recallsData.vin);
  ...
  const makeFragment = docRange.createContextualFragment(`
    <div class="${blockName}__recalls-md-row">
      <h5 class="${blockName}__recalls-md-title">${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('model year')}</h5>
      <span> ${recallsData.year}</span>
    </div>
    <div class="${blockName}__recalls-md-row">
      <h5 class="${blockName}__recalls-md-title">${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('make')}</h5>
      <span> ${recallsData.make}</span>
    </div>
    <div class="${blockName}__recalls-md-row">
      <h5 class="${blockName}__recalls-md-title">${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('model')}</h5>
  ...
  if (recallsData.recalls_available) {
    const listWrapperFragment = docRange.createContextualFragment(`
      <div class="${blockName}__recalls-heading-wrapper">
        ...
        <h4 class="${blockName}__recalls-heading" >${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('recalls')}  &nbsp; &nbsp;</h4>
        <p class="${blockName}__recalls-refresh-date"> [${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('recall_oldest_info')} ${recallsData.recalls_since}] </p>
      </div>
    `);
      ...
      valueDisplayList.forEach((item) => {
        if (recall[item.key]) {
          ...
          const itemFragment = docRange.createContextualFragment(`<li class="${blockName}__detail-item ${item.class ? item.class : ''}" >
            <h5 class="${blockName}__detail-title"> ${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)(item.key)} </h5>
    ...
    resultContent = `${resultContent} [${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('recall_available_info')} ${recallsData.recalls_since}]`;
  }
...
async function fetchRecalls(e) {
  ...
  if (e && e.target) {
    ...
    const resultText = document.querySelector(`.${blockName}__results-text`);
    resultText.innerText = (0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('loading recalls');
    ...
    if (vin) {
      try {
        const { url, key } = getAPIConfig();
        (0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getJsonFromUrl)(`${url}vin/${vin}?api_key=${key}&mode=company`)
          .then((response) => {
            if (response.error_code) {
              resultText.innerHTML = `${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('no recalls')} ${vin}`;
            } else {
...
async function decorate(block) {
  ...
  const refreshFragment = docRange.createContextualFragment(`<span>
    ${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('published_info')}:
    </span>
  ...
  const formChildren = document.createRange().createContextualFragment(`
    <div class="${blockName}__input-wrapper">
      ...
      />
      <label for="vin_number" class="${blockName}__label">${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('vinlabel')}</label>
    </div>
    <button class="button button--primary ${blockName}__submit" type="submit" name="submit">${(0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('submit')}</button>
  `);
  ...
  vinInput.oninvalid = (e) => {
    e.target.setCustomValidity((0,_scripts_common_js__WEBPACK_IMPORTED_MODULE_0__.getTextLabel)('vinformat'));
  };
...
/******/ var __webpack_exports__ = (__webpack_exec__("./blocks/vin-number/vin-number.js"));
/******/ }
]);